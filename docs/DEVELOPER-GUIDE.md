...
11. The UI can call `GET /api/learning/search?q=<term>&gameId=<id>` to recover learning traces by theme, explanation, SAN move, FEN, stored engine candidate, PGN ECO/opening, tags or written review attempts. The repository uses a local SQLite FTS5 index and falls back to bounded exact matching for unsupported query shapes.
...
- `GET /api/learning/search` accepts normal text, SAN/engine terms, a full FEN or a 64-character position hash. FEN/hash queries expand to exact matches and compatible position-family matches when the target position exists locally, returning `positionMatchScore` and `positionMatchReason` for explainable ranking.
